...
void init()
{
	beg.q_orientation = vec4(Y, -PI/1.9);
	end.q_orientation = vec4(Y, -PI/1.9);

	beg.cam_pos = -1e-3*X - 2.0*X;
	...
	win.screen_width = 320;
	win.screen_height = 320;
	win.n_frames = 64;
	win.ms_per_frame = 40;
...
void loop()
{
	...
	scene.q_orientation = quat(Y, mix(beg.q_orientation.w, end.q_orientation.w, pow(p, 0.2)));
